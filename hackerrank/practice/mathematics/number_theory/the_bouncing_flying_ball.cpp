...
int main_the_bouncing_flying_ball() {
  ...
  auto Add2 = [Add1](uint64_t L, uint64_t dL, uint64_t g, uint64_t p, int xp,
                     int xs, vector<uint64_t>& v) {
    Add1(L, dL, g, p, xp, xs, v);
    Add1(L, dL, g, p, -xp, xs, v);
  };

  auto Merge = [](uint64_t p1, uint64_t p2, uint64_t g, uint64_t p,
                  const vector<uint64_t>& v1, const vector<uint64_t>& v2,
                  vector<uint64_t>& v) {
    for (uint64_t x1 : v1) {
      for (uint64_t x2 : v2) {
  ...
  for (unsigned iT = 0; iT < T; ++iT) {
    ...
    for (const DInfo& info : vcurrent) {
      ...
      for (uint64_t x : info.vp1) p1 = min(x, p1);
      for (uint64_t x : info.vp2) p2 = min(x, p2);
      assert(p1 != p2);
      if (p1 < p2)
        s1 += count;
      else
