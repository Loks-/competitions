...
int main_period()
{
	...
	public:
		...
		AC(uint64_t _a, uint64_t _b, const TModularProxy& _p) : p(_p) { a = p.ApplyU(_a); b = p.ApplyU(_b); }
		AC operator*(const AC& r) const { return AC(a * r.a + 5 * b * r.b, a * r.b + b * r.a, p); }
		AC& operator*=(const AC& r) { AC t = *this * r; *this = t; return *this; }
	};
