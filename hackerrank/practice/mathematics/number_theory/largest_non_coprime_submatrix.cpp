...
using TTree = SegmentTree<unsigned, STInfoGCD<unsigned, STInfoSegment<>>, STActionNone, false>;
...
int main_largest_non_coprime_submatrix()
{
    ...
    for (unsigned i = 0; i < n; ++i)
        matrix[i] = ReadVector<unsigned>(m);
    ...
    for (unsigned i1 = 0; i1 < n; ++i1)
    {
        ...
        for (unsigned i2 = i1; i2 < n; ++i2)
        {
            for (unsigned j = 0; j < m; ++j)
                vrow[j] = GCD(matrix[i2][j], vrow[j]);
            TNode* root = st.BuildTree(vrow);
            for (unsigned j1 = 0, j2 = 1; j1 < m; )
            {
                unsigned gcd = STGetSegment(root, j1, j2 - 1).segment_gcd;
                if (gcd > 1)
                {
