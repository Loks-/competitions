...
int main_bear_and_cryptography() {
  ...
  std::function<bool(uint64_t, uint64_t, const vector<unsigned>&, unsigned,
                     unsigned)>
      SolveR = [&](uint64_t N, uint64_t M, const vector<unsigned>& powers,
                   unsigned power_index, unsigned min_prime_index) -> bool {
    ...
    for (;; ++min_prime_index) {
      ...
      first = false;
      if (M * N <= best_so_far) break;
      if (!b) break;
    }
    return true;
  };

  auto Solve = [&](uint64_t N, const vector<unsigned>& powers) {
    SolveR(N, 1, powers, 0, 0);
  };

  std::function<vector<vector<unsigned>>(unsigned, unsigned)> DivideK =
      [&](unsigned K, unsigned minv) -> vector<vector<unsigned>> {
    if (K == 1)
  ...
  for (unsigned it = 0; it < T; ++it) {
    ...
    if (best_so_far == 0) best_so_far -= 1;
