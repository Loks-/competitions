...
int main_sherlock_puzzle()
{
    ...
    for (int fk = max(0, maxk - 1); fk <= maxk; ++fk)
    {
        ...
        {
            auto it = lower_bound(vsmin.begin(), vsmin.end(), (fk + 1) * vs.back() + vs[i], [](int a, int b) { return a > b; });
            if (it != vsmin.end())
    ...
    if (maxl < s.size())
    {
        for (unsigned i = 0; i < vs.size(); ++i)
        {
            auto it = lower_bound(vsmin.begin(), vsmin.end(), vs[i], [](int a, int b) { return a > b; });
            if (it < vsmin.begin() + i)
