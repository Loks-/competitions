...
int main_grid_lines()
{
    ...
    auto G = [&](uint64_t n, uint64_t m, uint64_t k, uint64_t dx, uint64_t dy, uint64_t l)
    {
        return (f.BinomialCoefficient(l, k - 1) * n * m - 
            f.BinomialCoefficient(l + 1, k) * (n * dy + m * dx + dx * dy) * (k - 1) +
            f.BinomialCoefficient(l + 2, k + 1) * dx * dy * k * (k - 1)) * 2;
    };
    ...
    auto F = [&](uint64_t n, uint64_t m, uint64_t k)
    {
        ...
        for (uint64_t dx = 1; dx * (k - 1) < n; ++dx)
        {
            for (uint64_t dy = 1; dy * (k - 1) < m; ++dy)
            {
                ...
                s += G(n, m, k, dx, dy, min((n - 1) / dx, (m - 1) / dy)) - G(n, m, k, dx, dy, k - 2);
            }
        }
        return s;
    };
