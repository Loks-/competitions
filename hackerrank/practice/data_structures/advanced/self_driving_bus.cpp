  ...
  void PreDecompose(unsigned vertex, const CentroidDecomposition& cd) {
    ...
    candidates_r.push_back(make_pair(vertex, vertex));
    {
      ...
      for (unsigned j = vertex; j--;) {
        if (group_map[j] != current_group) break;
        req_min = min(req_min, path_min[j]);
        req_max = max(req_max, path_max[j]);
        if (req_min == j) candidates_l.push_back(make_pair(req_min, req_max));
      }
    }
    {
      // Go Right
      ...
      for (unsigned j = vertex + 1; j < n; ++j) {
        ...
        req_min = min(req_min, path_min[j]);
        req_max = max(req_max, path_max[j]);
        if (req_max == j) candidates_r.push_back(make_pair(req_min, req_max));
      }
    }
    ...
    for (size_t i = 0; i < candidates_l.size(); ++i) {
      size_t mink =
          lower_bound(cr_max.begin(), cr_max.end(), candidates_l[i].second) -
          cr_max.begin();
      size_t maxk =
          upper_bound(cr_min.begin(), cr_min.end(), n - candidates_l[i].first) -
          cr_min.begin();
      if (maxk > mink) total_segments += maxk - mink;
    }
    candidates_l.clear();
    candidates_r.clear();
  }
