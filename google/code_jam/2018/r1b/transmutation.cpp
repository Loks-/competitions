...
int main_transmutation() {
  ...
  for (unsigned it = 1; it <= T; ++it) {
    ...
    vector<uint64_t> available = nvector::Read<uint64_t>(M);
    uint64_t res = 0;
    ...
    for (bool stop = false; !stop;) {
      ...
      res += possible;
      for (unsigned i = 0; i < M; ++i) available[i] -= possible * required[i];
      for (unsigned i = 0; !stop && (i < M); ++i) {
        if (available[i] >= required[i]) continue;
        visited[i] = true;
        for (unsigned j = 0; j < M; ++j) {
          ...
          for (unsigned k = 0; k < M; ++k) {
            if (!visited[k]) receipts[k][j] += w * receipts[k][i];
          }
          required[j] += w * required[i];
          available[j] += w * available[i];
        }
        for (unsigned k = 0; k < M; ++k) {
          if (!visited[k]) receipts[k][i] = 0;
        }
