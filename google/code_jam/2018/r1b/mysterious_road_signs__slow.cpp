  ...
  for (unsigned it = 1; it <= T; ++it) {
    ...
    for (const auto it0 : point2intervals[0]) {
      ...
      for (const auto& interval : interval_list0) {
        ...
        if (interval.second < S) candidates.insert(targets[1][interval.second]);
      }
      ...
      for (int64_t candidate : candidates) {
        ...
        for (; (i0 < interval_list0.size()) && (i1 < interval_list1.size());) {
          auto current = (interval_list0[i0].first <= interval_list1[i1].first)
                             ? interval_list0[i0++]
                             : interval_list1[i1++];
    ...
    cout << "Case #" << it << ": " << best_intervals.max_length << " "
