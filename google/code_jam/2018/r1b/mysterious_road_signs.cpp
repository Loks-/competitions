...
namespace {
	...
	class MysteriousRoadSign
	{
		...
		void SolveC(unsigned begin, unsigned center, unsigned end, int64_t value0, int64_t value1)
		{
			...
			for (; i1 < end; ++i1)
			{
				if ((targets[0][i1] != value0) && (targets[1][i1] != value1))
					break;
			}
		...
		void SolveR(unsigned begin, unsigned end)
		{
			...
			if (l < best_intervals.max_length)
				return;
			if (l <= 2)
			{
			...
			for (unsigned j = 0; j < 2; ++j)
			{
				...
				if (value1r != value1l)
					SolveC(begin, m, end, value0, value1r);
	...
	public:
		void Solve(unsigned itest)
		{
			...
			SolveR(0, S);
			cout << "Case #" << itest << ": " << best_intervals.max_length << " " << best_intervals.intervals.size() << endl;
		}
	};
}
