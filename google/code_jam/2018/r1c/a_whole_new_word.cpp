	...
	for (unsigned it = 1; it <= T; ++it)
	{
        ...
        for (unsigned l = 0; l < L; ++l)
        {
            ...
            for (unsigned i : blocked_words)
                vc[words[i][l]] += 1;
            unsigned best = N + 1; char bestchar = 'A';
            for (char c = 'A'; c <= 'Z'; ++c)
            {
                if (!vb[c]) continue;
                if (vc[c] < best)
            ...
            for (unsigned i : blocked_words)
            {
                if (words[i][l] == bestchar)
                    blocked_words_next.push_back(i);
            }
            blocked_words.swap(blocked_words_next);
        }
		cout << "Case #" << it << ": " << (blocked_words.size() ? "-" : sfinal) << endl;
	}
	return 0;
}
