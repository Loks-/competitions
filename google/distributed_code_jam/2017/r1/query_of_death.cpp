...
int main_query_of_death() {
  ...
  int64_t first = block_id * length / blocks,
          last = (block_id + 1) * length / blocks, ln = last - first;

  if (block_id < blocks) {
    int master = 3 * block_id;
    if ((node_id % 3) == 1) {
      for (int64_t i = first; i < last; ++i) PutChar(master, GetValue(i));
      Send(master);
    } else if ((node_id % 3) == 2) {
      vector<char> v(ln);
      for (int64_t i = last - 1; i >= first; --i) v[i - first] = GetValue(i);
      for (char c : v) PutChar(master, c);
      Send(master);
    } else {
      // master
