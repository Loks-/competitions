...
int main_towels() {
  ...
  // Read data
  vector<vector<int64_t>> vtowels(ilast - ifirst,
                                  vector<int64_t>(towels_per_stack));
  ...
  for (int64_t i = 0; i < ilast - ifirst; ++i) {
    ...
    for (int64_t j = 0; j < towels_per_stack; ++j) {
      int64_t value =
          GetTowelCleanlinessRank(i + ifirst, towels_per_stack - j - 1);
  ...
  for (int current_master = 0; b - a > 1;
       current_master = (current_master + 1) % nodes) {
    int64_t c = (a + b) / 2;
    int64_t local =
                lower_bound(vtowels_merged.begin(), vtowels_merged.end(), c) -
                vtowels_merged.begin(),
            global;
  ...
  for (int64_t i = 0; i < ilast - ifirst; ++i) {
    for (int64_t j = 0; j < towels_per_stack; ++j) {
      if (vtowels[i][j] == a) {
        cout << GetTowelCleanlinessRank(i + ifirst,
                                        towels_per_stack - (j + index - at) - 1)
             << endl;
        return 0;
      }
    }
