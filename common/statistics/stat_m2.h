...
public:
    ...
    void Clear() { TBase::Clear(); s2 = TValue(0); }
    void AddSample(const TValue& x, const TValue& w = 1) { TBase::AddSample(x, w); s2 += x * x * w; }

    void MultWeight(const TValue& x) { TBase::MultWeight(x); s2 *= x; }
    void AddValue(const TValue& x) { s2 += x * x * TBase::s0 + 2 * x * TBase::s1; TBase::AddValue(x); }
    void AddStat(const TSelf& r) { TBase::AddStat(r); s2 += r.s2; }
    ...
    TValue CentralMoment2() const { TValue m = TBase::RawMoment1(); return RawMoment2() - m*m; }
