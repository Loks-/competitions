...
public:
    GraphEdgeConstIterator(const unsigned* _p1, const TEdgeInfo* _p2) : p1(_p1), p2(_p2) {}
    GraphEdge<TEdgeInfo> operator*() const { return {*p1, *p2}; }
    GraphEdgeConstIterator& operator++() { ++p1; ++p2; return *this; }
...
public:
    ...
    GraphEdgesFromVertex(const std::vector<unsigned>& _to, const std::vector<TEdgeInfo>& _info) : to(_to), info(_info) {}

    unsigned Size() const { return unsigned(to.size()); }
    const_iterator begin() const { return const_iterator(&to.front(), &info.front()); }
    const_iterator end() const { return const_iterator(&to.front() + Size(), &info.front() + Size()); }
};
