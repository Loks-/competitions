...
inline std::vector<TValue> TPM_PBST(const Tree<TGraph>& tree, const std::vector<TValue>& nodes_values, const std::vector<std::pair<unsigned, unsigned>>& paths, bool ignore_lca = false) {
  using TTree = bst::persistent::Treap<true, false, TValue, bst::info::segment::Max<TValue, bst::info::segment::None>>;
  ...
  for (s.push(std::make_pair(tree.GetRoot(), CNone)); !s.empty();) {
    ...
    s.pop();
    roots[v] = ptree.InsertNewNode((p == CNone) ? nullptr : roots[p], nodes_values[v], lca.deep[v]);
  ...
  for (auto& p : paths) {
    if ((p.first == p.second) && ignore_lca) {
      ...
      output.push_back(std::max(
        bst::info::segment::GetByKey(roots[p.first], lca.deep[a] + (ignore_lca ? 1 : 0), lca.deep[p.first] + 1).max,
        bst::info::segment::GetByKey(roots[p.second], lca.deep[a] + (ignore_lca ? 1 : 0), lca.deep[p.second] + 1).max));
    }
  }
  return output;
}
