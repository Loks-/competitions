...
template <class TBST>
inline void Rotate(typename TBST::TNode*& root, unsigned first, unsigned middle,
                   unsigned last) {
...
template <class TBST>
inline void RotateRight(typename TBST::TNode*& root, unsigned first,
...
// Rotate elements in interval [first, last) to right by shift
template <class TBST>
inline void RotateLeft(typename TBST::TNode*& root, unsigned first,
                       unsigned last, unsigned shift) {
  assert(last > first);
  shift %= (last - first);
  if (shift == 0) return;
