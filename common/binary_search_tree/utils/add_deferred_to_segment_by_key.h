...
/**
 * @brief Adds a deferred operation to all nodes in a segment defined by key
 * range.
 *
 * This function splits the tree into three parts based on the given key
 * range:
 * - Left part: nodes with keys less than 'begin'
 * - Middle part: nodes with keys in the range [begin, end)
 * - Right part: nodes with keys greater than or equal to 'end'
 *
 ...
 * @tparam TTree The tree type that provides split, join, and TNode types.
 ...
 * @param begin The beginning of the key range (inclusive).
 * @param end The end of the key range (exclusive).
...
[[nodiscard]] constexpr TNode* add_deferred_to_segment_by_key(
    TNode* root, const typename TTree::TKey& begin,
    const typename TTree::TKey& end, const TDeferredValue& deferred_value) {
  ...
  TTree::split(root, end, middle_part, right_part);

  // Split the middle part at 'begin' to get left and middle parts
  TTree::split(middle_part, begin, left_part, middle_part);
