...
/**
 * @brief Gets the leftmost leaf node in a subtree.
 *
 * This function traverses the leftmost path of a subtree to find the leaf node
 * with the smallest key. A leaf node is a node that has no children (both left
 ...
 * @return Pointer to the leftmost leaf node, or nullptr if the subtree is
 * empty.
 */
template <bool apply_deferred = true, typename TNode>
[[nodiscard]] constexpr TNode* left_leaf(TNode* root) {
  if (!root) return nullptr;

  // Traverse leftmost path until we find a leaf
  while (true) {
