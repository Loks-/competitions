...
namespace numeric {
constexpr uint64_t RotateBitsR(uint64_t x, unsigned shift) {
  shift &= 63;
  if (shift) x = ((((1ull << shift) - 1) & x) << (64 - shift)) | (x >> shift);
  return x;
}

constexpr uint64_t RotateBitsL(uint64_t x, unsigned shift) {
...
constexpr uint64_t RotateBitsL(uint64_t x, unsigned shift, unsigned length) {
