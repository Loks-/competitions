...
public:
	...
	TSelf& operator+=(const TSelf& r) { re += r.re; im += r.im; return *this; }
	TSelf& operator-=(const TSelf& r) { re -= r.re; im -= r.im; return *this; }
	...
	TSelf operator-(const TSelf& r) const { return TSelf(re - r.re, im - r.im); }
	TSelf operator*(const TSelf& r) const { return TSelf(re * r.re - im * r.im, re * r.im + im * r.re); }
	TSelf operator/(const TSelf& r) const
	{
		TValue tre = re * r.re + im * r.im, tim = im * r.re - re * r.im, td = r.re * r.re + r.im * r.im;
		assert(td != 0);
	...
	TSelf Inverse() const { TValue t = AbsSquared(); return TSelf(re / t, -im / t); }
