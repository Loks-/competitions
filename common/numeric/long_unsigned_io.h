...
inline LongUnsigned LongUnsignedParse(const std::string& s, unsigned base = 10)
{
    assert(base == 10);
    LongUnsigned lu;
    for (char c : s)
...
inline std::ostream& operator<<(std::ostream& s, const LongUnsigned& lu) { s << ToString(lu); return s; }

inline void Write(const LongUnsigned& lu, bool add_eod = true, unsigned base = 10)
{
