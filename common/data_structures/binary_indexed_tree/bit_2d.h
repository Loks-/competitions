...
class BIT_2D {
 ...
 public:
  ...
  constexpr BIT_2D(size_t _size1, size_t _size2)
      : size1(_size1), size2(_size2) {
    values.resize(size1 * size2);
  }

  constexpr void Reset(size_t new_size1, size_t new_size2) {
    size1 = new_size1;
    size2 = new_size2;
    values.resize(size1 * size2);
  ...
  constexpr void Add(size_t index1, size_t index2, TValue value = TValue(1)) {
  ...
  constexpr TValue Sum(size_t end1, size_t end2) const {
    TValue sum = TValue();
  ...
  constexpr TValue Sum(size_t b1, size_t b2, size_t e1, size_t e2) const {
    return Sum(e1, e2) - Sum(e1, b2) - Sum(b1, e2) + Sum(b1, b2);
  }

  constexpr TValue Get(size_t index1, size_t index2) const {
