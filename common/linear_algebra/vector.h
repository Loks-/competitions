...
public:
	...
	TSelf operator*(const TValue& v) const { TSelf t(*this); t *= v; return t; }
	TSelf operator/(const TValue& v) const { TSelf t(*this); t /= v; return t; }
	TSelf operator-() const { TSelf t(*this); for (iterator p = t.begin(), pend = t.end(); p < pend; ++p) *p = -*p;  return t; }

	TSelf& operator+=(const TSelf& v) { assert(Size() == v.Size()); const_iterator pv = v.begin(); for (iterator p = begin(), pend = end(); p < pend; ) *p++ += *pv++; return *this; }
	TSelf& operator-=(const TSelf& v) { assert(Size() == v.Size()); const_iterator pv = v.begin(); for (iterator p = begin(), pend = end(); p < pend; ) *p++ -= *pv++; return *this; }
