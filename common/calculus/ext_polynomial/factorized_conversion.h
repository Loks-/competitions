...
inline MVFunction<TValue, dim> ToFunction(const FactorizedLinear<TValue>& l) {
...
template <class TValue, unsigned dim>
inline MVFunction<TValue, dim> ToFunction(const Factorized<TValue>& f) {
  assert(f.SimpleD());
...
template <class TValue, unsigned dim>
inline MVFunction<TValue, dim> ToFunction(
