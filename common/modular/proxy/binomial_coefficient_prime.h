...
inline uint64_t PModularPrimeBinomialCoefficient(uint64_t n, uint64_t k, PModularFactorial<TModularProxy_P32U>& f)
{
    if (k > n) return 0;
...
inline uint64_t PModularPrimeBinomialCoefficient(uint64_t n, uint64_t k, unsigned prime)
{
