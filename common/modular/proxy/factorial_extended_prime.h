...
inline uint64_t PModularFactorialExtendedPrimeCoprimeOnly(
    uint64_t n, PModularFactorial<TModularProxy_P32U, false>& f) {
...
inline uint64_t PModularFactorialExtendedPrime(
    uint64_t n, PModularFactorial<TModularProxy_P32U, false>& f) {
  if (n == 0) return 1;
  const TModularProxy_P32U& mp = f.GetProxy();
  uint64_t p = mp.GetMod();
  return mp.Mult(PModularFactorialExtendedPrimeCoprimeOnly(n, f),
                 PModularFactorialExtendedPrime(n / p, f));
}
