...
inline TModular ModularFactorialExtendedPrimeCoprimeOnly(
    uint64_t n, ModularFactorial<TModular, true>& f, bool inverted = false) {
...
template <class TModular>
inline TModular ModularFactorialExtendedPrime(
    uint64_t n, ModularFactorial<TModular, true>& f, bool inverted = false) {
  if (n == 0) return 1;
  const uint64_t p = TModular::GetMod();
  return ModularFactorialExtendedPrimeCoprimeOnly(n, f, inverted) *
         ModularFactorialExtendedPrime(n / p, f, inverted);
}
