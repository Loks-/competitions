...
public:
	...
	static std::pair<TModular, TModular> GetPS(int64_t n)
	{
		...
		return (n & 1) ? std::make_pair(-p.second, p.first) : std::make_pair(p.second, -p.first);
	}

	static TModular GetU(uint64_t n) { return GetPU(n).second; }
	static TModular GetU(uint64_t n, TModular f0, TModular f1) { auto p = GetPU(n); return p.first * f0 + p.second * f1; }
	static TModular GetS(int64_t n) { return GetPS(n).second; }
	static TModular GetS(int64_t n, TModular f0, TModular f1) { auto p = GetPS(n); return p.first * f0 + p.second * f1; }
};
