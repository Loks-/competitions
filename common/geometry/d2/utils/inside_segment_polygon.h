...
namespace hidden {
template <class T>
inline bool InsideI(const Segment<T, true>& s, const Polygon<T>& plgn,
  ...
  if (l1.type == location::Location::VERTEX) {
    ...
    IAngle<T> as(s.p2 - p0), a1(p1 - p0), a2(p2 - p0);
  ...
  if (l2.type == location::Location::VERTEX) {
    ...
    IAngle<T> as(s.p1 - p0), a1(p1 - p0), a2(p2 - p0);
  ...
  for (unsigned i = 0; i < plgn.Size(); ++i) {
    Segment<T, false> s0(s.p1, s.p2), sp(plgn[i], plgn.MGet(i + 1));
...
template <class T>
inline bool Inside(const Segment<T, true>& s, const Polygon<T>& plgn) {
  for (auto& p : plgn.v) {
