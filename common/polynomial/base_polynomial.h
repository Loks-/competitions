...
public:
    ...
    void Normalize() { for (; (data.size() > 0) && (data.back() == TValue(0));) data.pop_back(); }

	TValue GetCoefficient(unsigned i) const { return (i < data.size()) ? data[i] : TValue(0); }
    ...
    TMe operator+(const TValue& v) const { TMe t(*Me()); t += v; return t; }
    TMe operator-(const TValue& v) const { TMe t(*Me()); t -= v; return t; }
    TMe operator*(const TValue& v) const { TMe t(*Me()); t *= v; return t; }
    TMe operator/(const TValue& v) const { TMe t(*Me()); t /= v; return t; }
    TMe operator-() const { TMe t(*Me()); for (TValue *p = t.begin(), *pend = t.end(); p < pend; ++p) *p = -*p;  return t; }
	...
	TMe& operator+=(const TMe& v)
    {
        if (Size() < v.Size())
            data.resize(v.Size());
        TValue* p = v.begin();
        for (const TValue *pv = v.begin(), *pvend = v.end(); pv < pvend; ) *p++ += *pv++;
        Normalize();
        return *Me();
    }
	...
	TMe operator+(const TMe& v) const { TMe t(*Me()); t += v; return t; }
	TMe operator-(const TMe& v) const { TMe t(*Me()); t -= v; return t; }
};
