...
public:
	...
	void InitSorted(const TSortedContainer& v) { InitIterator(v.begin(), v.end(), v.size()); }

	template<class TContainer>
	void InitUnsorted(const TContainer& v) { std::set<TValue> s(v.begin(), v.end()); InitSorted(s);	}

	unsigned Size() const { return unsigned(new_to_old.size()); }
	const TValue& GetOld(unsigned new_value) const { return new_to_old[new_value]; }
