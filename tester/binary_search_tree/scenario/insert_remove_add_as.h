...
class InsertRemoveAddAS : public Base<InsertRemoveAddAS<data_type>> {
 public:
  ...
  static constexpr std::string id() {
    return std::string("insert_remove_add_as @ ") + get_name(data_type);
  }
  ...
  static std::pair<size_t, std::chrono::nanoseconds> run_impl(
    ...
    for (unsigned i = 0, j = 0; j < size;) {
      if ((i < size) && (i - j < size / 2)) {
        // insert
        root = tree.insert_new(root, Data(keys[i]), keys[i]);
        ++i;
