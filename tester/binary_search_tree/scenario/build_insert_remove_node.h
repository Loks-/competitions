...
class BuildInsertRemoveNode : public Base<BuildInsertRemoveNode<data_type>> {
 public:
  static_assert(data_type == DataType::kIncreasing,
                "Only increasing data type is supported");
  ...
  static constexpr std::string id() {
    return std::string("build_insert_remove_node @ ") + get_name(data_type);
  }
  ...
  static std::pair<size_t, std::chrono::nanoseconds> run_impl(
    ...
    std::vector<typename Tree::NodeType*> nodes;
    for (unsigned i = 0; i < size; ++i)
      nodes.push_back(tree.create_node(Data(), keys[i]));

    // Build initial tree of size/4
    const size_t initial_size = size / 4;
    std::vector<typename Tree::NodeType*> initial_nodes(
        nodes.begin(), nodes.begin() + initial_size);
    ...
    // Start insert/remove_node operations from size/4
