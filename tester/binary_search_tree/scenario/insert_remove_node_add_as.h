...
class InsertRemoveNodeAddAS : public Base<InsertRemoveNodeAddAS<data_type>> {
 public:
  ...
  static constexpr std::string id() {
    return std::string("insert_remove_node_add_as @ ") + get_name(data_type);
  }
  ...
  static std::pair<size_t, std::chrono::nanoseconds> run_impl(
    ...
    for (unsigned i = 0; i < size; ++i)
      tree.create_node(Data(keys[i]), keys[i]);
    ...
    for (unsigned i = 0, j = 0; j < size;) {
      if ((i < size) && (i - j < size / 2)) {
        ...
        if constexpr (data_type != DataType::kShuffledDuplicates)
          hash_combine(hash, node->data.Get());
      } else {
        // remove
