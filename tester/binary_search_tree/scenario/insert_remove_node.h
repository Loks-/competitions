...
class InsertRemoveNode : public Base<InsertRemoveNode<data_type>> {
 public:
  ...
  static constexpr std::string id() {
    return std::string("insert_remove_node @ ") + get_name(data_type);
  }
  ...
  static std::pair<size_t, std::chrono::nanoseconds> run_impl(
    ...
    for (unsigned i = 0; i < size; ++i) tree.create_node(Data(), keys[i]);
